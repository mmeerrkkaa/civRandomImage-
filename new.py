...
def nick_roll(nick, nomer):
	...
	img = Image.open(f"./back/back_nick.jpg").convert("RGBA").resize((512, size[1]))
	...
	font = ImageFont.truetype("arial.ttf", size= round(50 - len(nick) / 7))
...
def nation_roll(country):
	...
	font_country = ImageFont.truetype("arial.ttf", size=65 - ((len(country)) // 2) * 5)
...
def roll_image(nick, count_nation, nomer, *country):
	...
	img = Image.new("RGBA", (512, size[1]*(count_nation+1)), "white")
	idraw = ImageDraw.Draw(img)
...
def rolls(nick, count_nation, *nation):
	...
	img = Image.new("RGBA", (width * 512, (size[1] * (count_nation + 1)) * lis), "white")
	idraw = ImageDraw.Draw(img)
